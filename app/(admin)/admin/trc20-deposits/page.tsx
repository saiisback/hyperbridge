...
export default function AdminTrc20DepositsPage() {
  ...
  const { data: deposits, isLoading, isFetching, page, totalPages, total, setPage, refetch } =
    useAdminData<Trc20Deposit>('/api/admin/trc20-deposits', 'deposits', {
      extraParams: { status: activeTab },
    })

  const handleApprove = async (deposit: Trc20Deposit) => {
    ...
    try {
      ...
      const res = await adminFetch(`/api/admin/trc20-deposits/${deposit.id}/approve`, accessToken, {
        method: 'POST',
      })
      ...
      if (res.ok) {
        toast({
          ...
          description: `${parseFloat(deposit.amount).toFixed(2)} USDT → ₹${parseFloat(data.conversion.amountInr).toLocaleString('en-IN', { minimumFractionDigits: 2 })} at rate ₹${parseFloat(data.conversion.rate).toLocaleString('en-IN')}`,
        })
        refetch()
      } else {
  ...
  const handleReject = async () => {
    ...
    try {
      const accessToken = await getAccessToken()
      if (!accessToken) return
      const res = await adminFetch(`/api/admin/trc20-deposits/${rejectingId}/reject`, accessToken, {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <Card className="bg-black/50 backdrop-blur-sm border-white/10 rounded-xl">
        <CardHeader>
          <CardTitle className="text-white">TRC-20 USDT Deposits</CardTitle>
        </CardHeader>
        <CardContent>
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="border-white/10 hover:bg-transparent">
                          ...
                          <TableHead className="text-white/50">Amount (USDT)</TableHead>
                          <TableHead className="text-white/50 hidden sm:table-cell">Sender</TableHead>
                          <TableHead className="text-white/50 hidden md:table-cell">Tx Hash</TableHead>
                          ...
                          return (
                            <TableRow key={d.id} className="border-white/10 hover:bg-white/5">
                              ...
                              <TableCell className="hidden sm:table-cell">
                                <code className="text-orange-500 font-mono text-xs break-all">
                                  {(meta?.senderAddress as string) || d.walletAddress || '-'}
                                </code>
                              </TableCell>
                              <TableCell className="hidden md:table-cell">
                                <code className="text-white/60 font-mono text-xs break-all">
                                  {d.txHash
                              ...
                              <TableCell>
                                ...
                                >
                                  {d.status === 'failed' ? 'Rejected' : d.status}
                                </Badge>
                              </TableCell>
                              ...
                              {activeTab === 'completed' && (
                                <TableCell className="text-white/70">
                                  {d.amountInr
                                    ? `₹${parseFloat(d.amountInr).toLocaleString('en-IN', { minimumFractionDigits: 2 })}`
                                    : '-'}
                                </TableCell>
                              )}
                            </TableRow>
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent className="bg-black/95 border-white/10">
          <DialogHeader>
            <DialogTitle className="text-white">Reject TRC-20 Deposit</DialogTitle>
            <DialogDescription className="text-white/50">
              This will reject the deposit. No balance was credited, so no refund is needed.
            </DialogDescription>
          </DialogHeader>
