...
import { TrendingUp, Users, Copy, Check, Calendar, DollarSign, Loader2 } from 'lucide-react'
...
interface IncomeData {
  ...
  referralEarnings: { level: number; members: number; percentage: string; earnings: number }[]
  referralHistory: { date: string; from: string; level: number; amount: number }[]
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-white mt-2">
                  {(data?.totalRoiIncome ?? 0).toFixed(4)} ETH
                </p>
                <p className="text-sm text-green-500 mt-1">
                  +{(data?.todayRoi ?? 0).toFixed(4)} ETH today
                </p>
              </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-white mt-2">
                  {(data?.totalReferralIncome ?? 0).toFixed(4)} ETH
                </p>
                <p className="text-sm text-orange-500 mt-1">
                  {data?.teamSize ?? 0} team members
                </p>
              </div>
      ...
      <Tabs defaultValue="roi" className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-bold text-white">
                      {(data?.totalInvested ?? 0).toFixed(4)} ETH
                    </p>
                  </div>
                </div>
              </CardContent>
        ...
        <TabsContent value="referral" className="space-y-6">
          ...
          <Card className="bg-black/50 backdrop-blur-sm border-white/10 rounded-xl">
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                {(data?.referralEarnings ?? []).map((level) => (
                  <div
                    key={level.level}
